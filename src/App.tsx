...
function App() {
  ...
  useEffect(() => {
    setLowValues(
      datas.filter(
        data => data.EC_slab1 || data.EC_slab2 || data.EC_drain_PC || data.time
      )
    );
    setMiddleValues(
      datas.filter(
        data => data.Rhair || data.WC_slab1 || data.WC_slab2 || data.time
      )
    );
    ...
    setMessValues(
      datas.filter(
        data => data.PipeGrow || data.PipeLow || data.Tout || data.time
      )
    );
    setSimilar1Values(
      datas.filter(data => data.HumDef || data.Tair || data.time)
    );
    setsimilar2Values(
      datas.filter(data => data.BlackScr || data.EnScr || data.time)
    );
  }, [datas]);
